...
void psCallback(const sensor_msgs::Range::ConstPtr& value) {
  if (countDist < NB_DIST_SENS)
    psValue[countDist]=((value->range - PS_OFFSET_SIMULATION[countDist]) < 0) ? 0 : (value->range - PS_OFFSET_SIMULATION[countDist]);
  countDist++;
}
...
int main(int argc, char **argv) {
  int i, psOffset[NB_DIST_SENS]={0,0,0,0,0,0,0,0}, Mode = 1;
  int oamOfmSpeed[2];
  double speed[2];
  ...
  ros::ServiceClient leftWheelPositionClient = n.serviceClient<webots_ros::set_float>(modelName+"/left_wheel_motor/set_position");
  leftWheelPositionClient.call(wheelSrv);
  ros::ServiceClient rightWheelPositionClient = n.serviceClient<webots_ros::set_float>(modelName+"/right_wheel_motor/set_position");
  rightWheelPositionClient.call(wheelSrv);
  ros::ServiceClient leftWheelVelocityClient = n.serviceClient<webots_ros::set_float>(modelName+"/left_wheel_motor/set_velocity");
  ros::ServiceClient rightWheelVelocityClient = n.serviceClient<webots_ros::set_float>(modelName+"/right_wheel_motor/set_velocity");
